...
spotless {
  kotlin {
    ...
    target("src/**/*.kt", "gradle/build-logic/src/**/*.kt")
  }
  kotlinGradle {
    ktfmt(libs.ktfmt.get().version).googleStyle()
    target("*.kts", "gradle/build-logic/*.kts")
  }
}
...
val generateDocTests =
  tasks.register<GenerateDocTests>("generateDocTests") {
...
kotlin.sourceSets.named("documentTest") {
  @Suppress("OPT_IN_USAGE") generatedKotlin.srcDir(generateDocTests)
}
...
tasks.clean {
  delete +=
    listOf(
      ...
      rootDir.resolve("site"), // Generated by MkDocs.
