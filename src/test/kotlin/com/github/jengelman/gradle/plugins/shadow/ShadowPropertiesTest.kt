...
class ShadowPropertiesTest {
  ...
  fun inheritManifestAttrsFromJars() =
    with(project) {
      ...
      tasks.jar.configure { it.manifest.attributes["jar"] = "fromJar" }
      val jar1 =
        tasks.register("jar1", Jar::class.java) { it.manifest.attributes["jar1"] = "fromJar1" }
      val jar2 =
        tasks.register("jar2", Jar::class.java) { it.manifest.attributes["jar2"] = "fromJar2" }
  ...
  fun inheritManifestMainClassFromJar() =
    with(project) {
      plugins.apply(JavaPlugin::class.java)
      tasks.jar.configure { it.manifest.attributes[mainClassAttributeKey] = "Main" }
      tasks.shadowJar.configure {
        it.mainClass.set("Main2") // This should not override the inherited one from jar.
      }
      assertThat(tasks.shadowJar.get().manifest.attributes)
    ...
    with(project) {
      ...
      with(shadowJarTask as Jar) {
        ...
        assertThat(description)
          .isEqualTo("Create a combined JAR of project and runtime dependencies")
      ...
      with(shadowJarTask) {
        ...
        assertThat(configurations.get()).containsOnly(runtimeConfiguration.single())
      }
    }
  ...
  fun applyApplicationPlugin() =
    with(project) {
      ...
      with(runShadowTask) {
        assertThat(description)
          .isEqualTo("Runs this project as a JVM application using the shadow jar")
      ...
      with(startShadowScripts) {
        ...
        assertThat(classpath?.files)
          .isNotNull()
          .containsOnly(shadowJarTask.archiveFile.get().asFile)
        assertThat(mainModule.orNull).isEqualTo(applicationExtension.mainModule.orNull)
        assertThat(mainClass.orNull).isEqualTo(applicationExtension.mainClass.orNull)
  ...
  private companion object {
    ...
    val Task.dependsOnTaskNames: List<String>
      get() = dependsOn.filterIsInstance<Named>().map(Named::getName)
