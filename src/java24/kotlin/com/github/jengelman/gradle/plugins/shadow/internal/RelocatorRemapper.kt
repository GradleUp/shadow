...
internal fun FileCopyDetails.remapClass(relocators: Set<Relocator>): ByteArray =
  file.readBytes().let { bytes ->
    var modified = false
    val remapper = ClassFileRelocatorRemapper(relocators) { modified = true }
...
private class ClassFileRelocatorRemapper(
  ...
  private fun <S : Signature> S.remap(): S {
    return when (this) {
      ...
      is Signature.ClassTypeSig -> {
        ...
        val mappedInternalName =
          mappedClass.descriptorString().let { it.substring(1, it.length - 1) }
        val className =
          mappedOuter?.let { outer ->
            val mappedOuterDesc = outer.classDesc().descriptorString()
            val mappedOuterName = mappedOuterDesc.substring(1, mappedOuterDesc.length - 1)
            // With an outer type, ClassTypeSig expects only the nested class segment. Passing the
            // full internal name would produce an invalid signature such as
            // Lpkg/Outer<TT;>.pkg/Outer$Inner;.
            mappedInternalName.removePrefix("$mappedOuterName$").takeIf {
              it != mappedInternalName
            } ?: mappedInternalName.substringAfterLast('$', className())
          } ?: mappedInternalName
