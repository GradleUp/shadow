    ...
    implements Filter
{
    ...
    public MinijarFilter( Project project, Logger log )
        throws IOException
    {
    ...
    public MinijarFilter( Project project, Logger log, List<SimpleFilter> simpleFilters )
        throws IOException
    {
        ...
        ClazzpathUnit artifactUnit =
            cp.addClazzpathUnit( new FileInputStream( project.getArtifacts().getFile() ), project.toString() );

        for ( Iterator it = project.getConfigurations().getDependencies().iterator(); it.hasNext(); )
        {
