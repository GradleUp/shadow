 ...
 * - Duplicate `.class` files Having duplicate `.class` files with different content is a situation
 *   indicating that the resulting jar is built with _incompatible_ classes, likely leading to
 *   issues during runtime. This situation can happen when one dependency is (also) included in an
 *   uber jar.
 * - Duplicate `META-INF/<group-id>/<artifact-id>/pom.properties`/`xml` files. Some dependencies
 *   contain shaded variants of other dependencies. Tools that inspect jar files to extract the
 *   included dependencies, for example, for license auditing use cases or tools that collect
 *   information of all included dependencies, may rely on these files. Hence, it is desirable to
 *   retain the duplicate resource `pom.properties`/`xml` resources.
 *
 * [DeduplicatingResourceTransformer] checks all entries in the resulting jar. It is generally not
 * recommended to use any of the [include] configuration functions.
 *
 ...
 * `META-INF/<group-id>/<artifact-id>/pom.properties`/`xml`, configure the transformer with an
 * exclusion like the following:
 * ```kotlin
 * tasks.shadowJar {
 ...
 *
 * *Warning* Do **not** combine [PreserveFirstFoundResourceTransformer] with this transformer, as
 * they handle duplicates differently and combining them would lead to redundant or unexpected
 * behavior.
 */
...
public open class DeduplicatingResourceTransformer(
  ...
  @get:Internal internal val sources: MutableMap<String, PathInfos> = mutableMapOf()
  ...
  internal fun duplicateContentViolations(): Map<String, PathInfos> =
    sources.filter { (_, pathInfos) ->
