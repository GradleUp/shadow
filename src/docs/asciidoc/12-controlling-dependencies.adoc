...
****
Note the literal use of `project.configurations` when setting the `configurations` attribute of a `ShadowJar` task.
...
Individual dependencies can be filtered from the final JAR by using the `dependencies` block of a `ShadowJar` task.
...
[source,groovy,indent=0]
----
include::{tests}/FilteringSpec.groovy[tags=excludeProject1;excludeProject2]
----
...
If more complex decisions are needed to select the dependencies to be included, the `dependencies` block provides a
method that accepts a `Closure` for selecting dependencies.

.Selecting Dependencies to Filter with Spec
[source,groovy,indent=0]
----
include::{tests}/FilteringSpec.groovy[tags=excludeSpec;excludeSpec2]
----
